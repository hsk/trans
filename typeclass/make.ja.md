...
## 5 さらなる例： 等価性
  ...
  このインスタンスの最初の行には、 "`a` がクラス `Eq` にあり、 `b`がクラス `Eq` にあるようなすべての`a`と`b`について、ペア `(a,b)` もクラス `Eq` にあります"。
  ...
  集合の等価性が `member` の項で定義され、 `member` はオーバーロードされた等価性を使用しているので、 整数、整数のリスト集合、および整数の組の偶数組の集合に等価性を適用することが有効です。
...
### 5.1 等価性の変換
  ...
  インスタンス `Eq Int` と `Eq Char` に対応して、適切な等価関数を含む `EqD Int` 型と `EqD Char` 型の2つの辞書を定義し、 関数メンバを直接 '`member`' に変換します。
