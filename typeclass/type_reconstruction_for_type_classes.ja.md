...
## 2.3 型の分類
  ...
  `Γ'` は `Γ` よりも一般的です。
  `⊢` が単調であることを示すのは簡単です。この順序付け: `Σ,Γ' ⊢ τ : S` は `Σ,Γ ⊢ τ : S` を意味します。
...
<!-- page 8 -->
  ...
  型スキーム `σ` は、式 `e` w.r.t の主な型です。 `Σ,[],E ⊢ e : σ`、`Σ,[],E ⊢ e : σ` を用いたすべての `σ` に対して、`Σ` と `σ'` は `σ` の一般的なインスタンス、すなわち `Σ,[] ⊢ σ ⪰ σ` でなければなりません。
  ...
  TODO: 見えない文字対応
  TODO: w.r.tの箇所をWRTに書き換えてみる。w.r.t.の意味を調べる。
...
## 4 ソート制約付きの型の単一化
  ...
  原則として、この問題は(NS91)w.r.tで行われているように、オーダーソートされた単一化に減らすことができます。 `⪯`。
...
<!-- page 15 -->
  ...
  次の補題は、 `Γ ⊢ θ : Γ′` という関係が一種の過渡性を持つことを示しています。置換。
...
<!-- page 17 -->
  ...
  我々は`Σ,Γ*,θ* E ▷ e : τ*` の導出のw.l.o.g.を仮定します。 アルゴリズムWによって使用される新しい変数を生成し `V1 - V` は変数が重なりません。
...
<!-- page 19 -->
  ...
  `{(αk*)~} ∩ FV(θ* E) = {}` として、我々は w.l.o.g. `{(αk*)〜}∩Dom(Γ*)= {}`と仮定するために、上記のルールの前提(`Γ'` ではなく)の`(αk*)~`
  ...
  ここで、δ1=δ1|(Dom(δ1) - {(αn)〜}) `とすると、δ1∀(αn:Sn)~θ1τ1=∀(αn:Sn)~δ1θ1τ1 ' Wによって使用される新しい変数は、選択された導出、すなわち `Cod(δ1)`には現れません。
  次に、δ1|σ2を用いて、Σ、Γ*⊢δ1∀(αn:Sn)〜.θ1τ1⪰∀(αk*:Sk *)〜τ*定義3.1の中の代入として{{(αn)〜}を代入し、 `Σ、Γ* [(αk*:Sk *)〜]†δ1αi:Si`、` [i = 1 ... n] `は25)。

  ここで問題となるのは、誘導仮説は、一般に `(θ* E)[x:0]であるため、Γ*⊢δ1:Γ'1とΘ1E [...] (α1:σ1)=(δ1∈E)[x:δ1∀(αn:Sn)〜.θ1τ1]です。
  したがって、e2の帰納仮説を適用するためには、異なる根拠を見つけなければなりません。
  TODO^^^
...
<!-- page 20 -->
  ...
  **証明** いくつかの型の `Σ,[],E ⊢ e : ∀(αm':Sm')~ .τ'` を仮定します。
  ...
  このようにして、`δ` が得られ
   ...
   そして、 `∀(αn : Γαn)~ .θτ` は `e` w.r.t. `E` の主要な型です。 `τ' = δθτ`, `{(αn)~} = FV(θτ)` から `τ' = δθτ`、 `{(αn)~} = FV(θτ)`、 `Γ* ⊢ δ : Γ` です。 □
...
<!-- page 23 -->
  ...
  この議論から、型システムの意味論と一貫性の証明は明らかです。セマンティクスが緊急に必要とされています。
